...
public:
    ...
    bool open1(bool visible = false);
    bool open(bool visible = false, int mode = Excel::ReadWrite);
    bool open(const QString &name, bool visible = false, int mode = Excel::ReadWrite);
    void close();
    bool isOpen();
    void save();
    void saveAs(const QString &name);
    void setDisplayAlerts(bool visible = false);
    // read
    QVariant readAll();
