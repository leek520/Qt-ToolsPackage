...
bool QtExcel::open1(bool visible)
{
...
bool QtExcel::open(bool visible, int mode)
{
    if(!m_excel){
        m_excel = new QAxObject("Excel.Application", this);
    }
    ...
    if(!QFile::exists(m_fileName)){
        ...
        if (mode & Excel::Truncate){
            ...
            m_workBooks->querySubObject("Open(const QString&)", m_fileName);
            m_workBook = m_excel->querySubObject("ActiveWorkBook");
            m_sheets = m_workBook->querySubObject("WorkSheets"); // or "Sheets"
        }
    }

    m_sheet = m_workBook->querySubObject("Sheets(int)", 1);

    m_bOpened = true;
    return true;
}
bool QtExcel::open(const QString &name, bool visible, int mode)
{
    m_fileName = name;
    return open(visible, mode);
}
void QtExcel::close()
{
    if(m_workBook){
...
void QtExcel::setDisplayAlerts(bool visible)
{
   m_excel->setProperty("DisplayAlerts", visible);
}
